...
export class PerformanceOptimizer {
  constructor(config = {}) {
    ...
    try {
      ...
      if (!this.cache || typeof this.cache.get !== 'function') {
        throw new Error('cache Map初始化失败');
      }
      if (!this.timers || typeof this.timers.get !== 'function') {
        throw new Error('timers Map初始化失败');
      }
      if (!this.throttledFunctions || typeof this.throttledFunctions.get !== 'function') {
        throw new Error('throttledFunctions Map初始化失败');
      }
      
      console.log('✅ [性能优化器] Map对象初始化成功');
    } catch (error) {
      console.error('❌ [性能优化器] Map对象初始化失败:', error);
      throw new Error(`性能优化器初始化失败: ${error.message}`);
    }
  ...
  async optimizeLayoutExecution(layoutFunction, context = {}, options = {}) {
    ...
    try {
      ...
      if (this.config.enableSmartCache) {
        ...
        if (cachedResult) {
          // 🔧 修复：添加安全检查
          if (this.state && this.state.performanceMetrics) {
  ...
  async handleBatchedExecution(layoutFunction, context, options, operationId) {
    ...
    if (currentQueueLength >= this.config.batchSize) {
      // 清除现有的延迟执行定时器，避免重复执行
      if (this.timers && typeof this.timers.has === 'function' && this.timers.has('batchExecution')) {
        const timer = this.timers.get('batchExecution');
        if (timer) {
          clearTimeout(timer);
        }
    ...
    if (this.timers && typeof this.timers.has === 'function' && !this.timers.has('batchExecution')) {
      const timer = setTimeout(() => {
        ...
        this.processBatch();
        if (this.timers && typeof this.timers.delete === 'function') {
          this.timers.delete('batchExecution');
        }
      }, this.config.batchDelay);
      
      if (this.timers && typeof this.timers.set === 'function') {
  ...
  optimizePreviewLineUpdates(updateFunction, context = {}) {
    ...
    if (this.throttledFunctions && typeof this.throttledFunctions.has === 'function' && this.throttledFunctions.has(throttleKey)) {
    ...
    if (this.throttledFunctions && typeof this.throttledFunctions.set === 'function') {
  ...
  checkCache(layoutFunction, context, options) {
    ...
    // 检查缓存
    if (this.cache && typeof this.cache.has === 'function' && this.cache.has(cacheKey)) {
      console.log(`📦 [缓存命中] 使用缓存结果: ${cacheKey}`);
  ...
  safeDeepCopy(obj, maxDepth = 5) {
    ...
    const copy = (value, depth = 0) => {
      ...
      // 检查循环引用
      if (seen && typeof seen.has === 'function' && seen.has(value)) {
      ...
      if (Array.isArray(value)) {
        result = [];
        if (seen && typeof seen.set === 'function') {
  ...
  updatePerformanceMetrics(executionTime) {
    // 🔧 修复：添加安全检查
    if (!this.state || !this.state.performanceMetrics) {
      console.warn('⚠️ [性能指标] state或performanceMetrics未初始化，跳过更新');
      return;
    }
