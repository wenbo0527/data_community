...
const applyFilter = async () => {
  await store.fetchContractList({ page: 1, pageSize: pageSize.value, supplier: filters.supplier, status: filters.status })
...
const filterBySupplier = async (supplier: string) => {
...
const submitSupplier = async () => {
  ...
  if (!valid) return
  supplierRegistry.value.unshift({ name: supplierForm.name, description: supplierForm.description })
...
const daysToExpire = (endDate?: string) => {
  try {
    const now = new Date()
...
const expireTagStatus = (endDate?: string) => {
  const days = daysToExpire(endDate)
...
const supplierSummary = computed(() => {
  const map = new Map<string, { supplier: string; count: number; totalAmount: number; expiringCount: number }>()
  list.value.forEach((i: ContractItem) => {
    const key = i.supplier || '—'
    const days = daysToExpire(i.endDate)
    const bucket = map.get(key) || { supplier: key, count: 0, totalAmount: 0, expiringCount: 0 }
    bucket.count += 1
    bucket.totalAmount += Number(i.amount) || 0
    if (!isNaN(days) && days <= 30 && days >= 0) bucket.expiringCount += 1
    map.set(key, bucket)
  })
...
onMounted(async () => {
  await store.fetchContractList({ page: current.value, pageSize: pageSize.value })
  pagination.total = store.total
})
</script>
