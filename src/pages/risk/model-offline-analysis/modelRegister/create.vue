<template>
  <div class="model-create-page">
    <div class="page-header">
      <a-breadcrumb>
        <a-breadcrumb-item to="/risk/model-offline-analysis/model-register">模型注册</a-breadcrumb-item>
...
import { modelAPI, featureAPI } from '@/api/offlineModel'
...
const handleCancel = () => goBack(router, '/risk/model-offline-analysis/model-register')
...
const runTest = () => {
  try {
    const inputData = JSON.parse(testForm.value.input || '{}')
    console.log('执行测试，输入数据:', inputData)
    testMsg.value = `测试成功，输出: ${JSON.stringify({ score: Math.random().toFixed(4) })} (模拟)`
  } catch (e) {
    Message.error('测试输入JSON格式错误')
  }
}
</script>

<style scoped>
.model-create-page { padding: 16px; background: #f0f2f5; min-height: 100vh; }
.page-header { margin-bottom: 16px; }
.page-title { margin: 8px 0 0; font-size: 20px; font-weight: 600; }
.page-content { padding-bottom: 80px; }
.panel-card { margin-bottom: 16px; }
.code-block { background: #f5f5f5; padding: 12px; border-radius: 4px; font-family: monospace; white-space: pre-wrap; }
.actions-bar { position: fixed; bottom: 0; right: 0; left: 200px; padding: 12px 24px; background: #fff; box-shadow: 0 -2px 8px rgba(0,0,0,0.06); z-index: 100; }
.actions-inner { display: flex; justify-content: flex-end; gap: 12px; }
</style>
