...
const columns = [
  ...
  },
  {
    ...
    width: 100,
    render: ({ record }: { record: { messageType: string } }) => {
  ...
  },
  {
    title: '策略Tag',
    dataIndex: 'tags',
    render: ({ record }: { record: { tags?: string[] } }) => {
      return h('div', record.tags?.map((tag: string) => 
        h(ATag, { color: 'blue', style: { marginRight: '4px' } }, () => tag)
      ))
    }
  },
  {
...
// 查看详情
const viewDetail = (record: { id: string }) => {
  console.log('查看详情', record)
}

// 测试模板
const testTemplate = (record: { id: string }) => {
