...
const buildProductsView = () => { const list = Array.isArray(products.value) ? products.value : []; const mapStatus = (s?: string) => { if (s === 'active') return 'online'; if (s === 'inactive') return 'archived'; if (s === 'pending_evaluation') return 'pending_evaluation'; if (s === 'importing') return 'importing'; return (s || 'online') }; productsView.value = list.map((p: any, idx: number) => ({ id: String(p.id ?? idx + 1), name: p.name || p.productName || p.code || `外数产品-${idx+1}`, code: p.code || `ED-${idx+1}`, supplier: p.supplier || p.provider || '—', status: mapStatus(p.status), createdAt: p.createdAt || randomDateWithinYear(), usageScene: p.usageScene || '贷前评分/贷中监控', billingMode: p.billingMode || 'per_call', unitPrice: typeof p.unitPrice === 'number' ? p.unitPrice : randBetween(1, 10), billingCycle: p.billingCycle || 'month', currency: p.currency || 'CNY', effectiveDate: p.effectiveDate || new Date(Date.now() - 30 * 86400000).toISOString(), expireDate: p.expireDate || new Date(Date.now() + 335 * 86400000).toISOString(), tags: Array.isArray(p.tags) ? p.tags : ['外数','风控'], evaluationScore: p.evaluationScore ?? randBetween(60, 95), monitorStatus: p.monitorStatus || (Math.random() > 0.15 ? '正常' : '异常') })); if (!productsView.value.length) { const seed = [{ id: 1, name: '产品A', provider: '数据提供商A', status: 'active' },{ id: 2, name: '产品B', provider: '数据提供商B', status: 'active' },{ id: 3, name: '产品C', provider: '数据提供商C', status: 'inactive' },{ id: 4, name: '产品D', provider: '数据提供商D', status: 'active' }]; productsView.value = seed.map((p: any, idx: number) => ({ id: String(p.id ?? idx + 1), name: p.name, code: `ED-${idx+1}`, supplier: p.provider, status: mapStatus(p.status), createdAt: randomDateWithinYear(), usageScene: '贷前评分/贷中监控', billingMode: 'per_call', unitPrice: randBetween(1, 10), billingCycle: 'month', currency: 'CNY', effectiveDate: new Date(Date.now() - 30 * 86400000).toISOString(), expireDate: new Date(Date.now() + 335 * 86400000).toISOString(), tags: ['外数','风控'], evaluationScore: randBetween(60, 95), monitorStatus: Math.random() > 0.15 ? '正常' : '异常' })); } pagination.total = productsView.value.length }
