...
const onGranularityChange = async (key: 'month' | 'quarter') => {
  ...
  await monitorStore.fetchBurndown({ range: key, businessType: filterForm.businessType || undefined, platform: filterForm.platform || undefined, targetLoan: typeof filterForm.targetLoan === 'number' ? filterForm.targetLoan : undefined, year: filterForm.year || undefined } as any)
}

const warningData = ref<WarningItem[]>([])
const platformOptions = computed<string[]>(() => { const set = new Set<string>(); budgetStore.list.forEach((i: any) => set.add(i.platform)); warningData.value.forEach((i: any) => i.platform && set.add(i.platform)); return Array.from(set) })
const targetLoanOptions = computed<number[]>(() => { const set = new Set<number>(); budgetStore.list.forEach((i: any) => typeof i.targetLoan === 'number' && set.add(i.targetLoan)); warningData.value.forEach((i: any) => typeof i.targetLoan === 'number' && set.add(i.targetLoan)); return Array.from(set).sort((a, b) => a - b) })
const yearOptions = Array.from({ length: 5 }, (_, i) => (new Date().getFullYear() - i).toString())
const filteredWarningData = computed(() => { let rows = warningData.value; if (filterForm.businessType) rows = rows.filter((r: any) => r.businessType === filterForm.businessType); if (filterForm.platform) rows = rows.filter((r: any) => r.platform === filterForm.platform); if (typeof filterForm.targetLoan === 'number') rows = rows.filter((r: any) => Number(r.targetLoan) === Number(filterForm.targetLoan)); if (filterForm.year) rows = rows.filter((r: any) => (r.year ? String(r.year) === String(filterForm.year) : true)); return rows })

const getHealthStatus = (r: any) => { const overCost = r.actualCost > r.estimatedCost * 1.1; const slowConsume = r.actualLoan < r.estimatedLoan * 0.9; if (!overCost && !slowConsume) return '正常'; if (overCost) return '超支'; return '消耗过慢' }
