    ...
    <a-space direction="vertical" fill style="width: 100%">
      <a-card title="固定费用产品核算" :bordered="true">
        <a-space style="margin-bottom: 8px">
          <a-button type="primary" size="小" :disabled="fixedLines.length===0" @click="confirmBatch('fixed')">批量确认</a-button>
        </a-space>
        <a-table 
            ...
            <a-table-column title="操作" :width="200">
              <template #cell="{ record }">
                <a-space>
                  ...
                  <a-button size="small" type="text" :disabled="excluded[record.lineId]" @click="confirmOne(record.lineId)">确认</a-button>
            ...
            <a-table-column title="操作" :width="220">
              <template #cell="{ record }">
                <a-space>
                  <a-checkbox v-model="excluded[record.lineId]">剔除</a-checkbox>
                  <a-button v-if="excluded[record.lineId]" size="small" type="text" @click="excluded[record.lineId]=false">恢复</a-button>
                  <a-button size="small" type="text" :disabled="excluded[record.lineId]" @click="confirmSpecial(record.lineId)">确认</a-button>
                  <a-tag :status="confirmed[record.lineId] ? 'success' : 'default'">{{ confirmed[record.lineId] ? '已确认' : '待确认' }}</a-tag>
                </a-space>
              </template>
            </a-table-column>
...
import { createSettlementTask } from '@/modules/budget/api/settlement'
...
const confirmOne = (lineId: string) => { confirmed.value[lineId] = true }
...
watch(() => props.supplierId, (sid: string | undefined) => {
  ...
  if (form.value.supplierId && form.value.month) loadBills()
})
watch(() => props.month, (m: string | undefined) => {
  form.value.month = m || `${new Date().getFullYear()}-${String(new Date().getMonth()+1).padStart(2,'0')}`
  if (form.value.supplierId && form.value.month) loadBills()
})
watch(() => [form.value.supplierId, form.value.month], ([sid, m]: [string, string]) => {
  if (!embedded.value && sid && m) loadBills()
})
