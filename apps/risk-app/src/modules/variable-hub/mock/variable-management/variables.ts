...
export const variableAssets: VariableAssetMock[] = [
  ...
  // ============ 补齐：online（已上线）============
  {
    ...
    apiName: 'midloan_contact_avg_30d_query',
    onlineTime: fmt(new Date(now.getTime() - 1 * 86400000)),
    developingOaAt: fmt(new Date(now.getTime() - 4 * 86400000)),
    pendingVerifyAt: fmt(new Date(now.getTime() - 3 * 86400000)),
    syncingInternalAt: fmt(new Date(now.getTime() - 2 * 86400000)),
    syncingVariableAt: fmt(new Date(now.getTime() - 1.5 * 86400000)),
    /** 当前状态=已上线（H1 R05：变量中心确认后填入引用信息）*/
    referenceStatus: '已引用',
    referenceDetail: '已正式投产到变量中心，关联决策引擎：风控V3、风控V4',
  ...
  // ============ 补齐：syncing_variable（同步中 变量中心）============
  {
    ...
    apiName: 'midloan_gps_abnormal_30d_query',
  ...
  // ============ 补齐：offline（已下线）============
  {
    ...
    apiName: 'midloan_bigtxn_30d_v1_query',
  ...
  },
  {
    ...
    apiName: 'midloan_multiloan_query_90d_query',
    ...
    costMetrics: { pricePerCall: 0.05, monthlyCalls: 0, monthlyCost: 0, costTrend: 'stable' }
  },
  // ============ 补齐：offline（已下线）============
  {
    ...
    apiName: 'midloan_fp_change_7d_query',
  ...
  // ============ 补齐：variable_sync_failed（变量中心同步失败）============
  {
    ...
    apiName: 'midloan_neg_word_60d_query',
    /** 当前状态=变量中心同步失败（D.3 时间戳）*/
    developingOaAt: fmt(new Date(now.getTime() - 7 * 86400000)),
    pendingVerifyAt: fmt(new Date(now.getTime() - 3 * 86400000)),
    syncingInternalAt: fmt(new Date(now.getTime() - 1.5 * 86400000)),
  ...
  // ============ 补齐：dw_online_failed（数仓上线失败）============
  {
    ...
    costMetrics: { pricePerCall: 0, monthlyCalls: 48000, monthlyCost: 0, costTrend: 'stable' }
  },
  // ============ 补齐：offline_failed（下线接收失败）============
  {
    ...
    apiName: 'midloan_card_change_7d_query',
  ...
  // ============ 补齐：Boolean / String 字段类型 + 多样数据时效 ============
  {
    ...
    apiName: 'midloan_remote_login_30d_query',
    onlineTime: fmt(new Date(now.getTime() - 15 * 86400000)),
    referenceStatus: '已引用',
    referenceDetail: '已上线到变量中心，关联决策引擎：风控V3、风控V4',
    upstreamTable: 'dwd_login_log',
    effectMetrics: { iv: 0.28, ks: 0.22, auc: 0.7, coverage: 0.97, lift: 9 },
    costMetrics: { pricePerCall: 0, monthlyCalls: 110000, monthlyCost: 0, costTrend: 'stable' }
  },
  {
