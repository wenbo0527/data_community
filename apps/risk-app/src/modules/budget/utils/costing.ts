import { getMonthlyUsageBySupplier } from '../../budget/api/usage'
import type { UsageRecord } from '../../budget/api/usage'
import { getActivePricing } from '../../budget/api/pricingArchive'
import type { Pricing } from '../../budget/api/pricingArchive'
...
function calcTieredAmount(qty: number, tiers: NonNullable<Pricing['tiers']>): number {
  ...
  for (const t of tiers) {
    ...
    if (span > 0) { total += span * t.price; remain -= span }
    if (remain <= 0) break
  }
...
function calcAmountByPricing(qty: number, pricing: Pricing): number {
  if (pricing.billingType === 'special') return 0
  if (pricing.billingType === 'fixed' || pricing.billingType === 'volume') return qty * (pricing.unitPrice || 0)
  if (pricing.billingType === 'tiered' && pricing.tiers?.length) return calcTieredAmount(qty, pricing.tiers)
...
export async function generateBillLines(supplierId: string, month: string): Promise<BillLine[]> {
  ...
  const { useContractStore } = await import('../stores/contract')
