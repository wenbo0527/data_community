...
function addMappingRow() { mappingList.value.push({ __id: `M-${Date.now()}-${Math.random()}`, externalProductCode: '', externalProductName: '', aliases: ['', '', ''] }) }
...
function onMappingUpload(files: any) {
  try {
    ...
    reader.onload = () => {
      ...
      recs.forEach((r: any) => {
        mappingList.value.push({ __id: `M-${Date.now()}-${Math.random()}`, externalProductCode: String(r.externalProductCode || ''), externalProductName: String(r.externalProductName || ''), aliases: [String(r.alias1 || ''), String(r.alias2 || ''), String(r.alias3 || '')] })
      })
...
const hasCosting = computed(() => { if (!form.value.supplierId || !form.value.month) return false; return Boolean(flowStore.getCosting(form.value.supplierId, form.value.month)) })
const goToCosting = () => {
  const sid = form.value.supplierId || ''
  const mon = form.value.month || `${new Date().getFullYear()}-${String(new Date().getMonth()+1).padStart(2,'0')}`
  router.push(`/budget/accounting?stage=costing&supplierId=${encodeURIComponent(sid)}&month=${encodeURIComponent(mon)}`)
}
...
function normalizeHeader(h: string): string {
  const map: Record<string, string> = { '账单周期': 'period', '产品名称': 'productName', '朴道产品代码': 'productCode', '计费方式': 'billingType', '最终计费次数': 'finalCount', '减免次数': 'discountCount', '最终付费次数': 'finalPaidCount', '单价': 'unitPrice', '金额': 'amount' }
  return map[h] || h
}
...
const buildRows = async () => {
  ...
  rows.value = externalRows.value.map((e: any) => {
    ...
    return { productCode: code, productName: name || (supplierProducts.value.find((p: any) => p.productCode === code)?.productName || code), matched: Boolean(pricingMap.value[code]), billingTypeLabel, systemUsage: sys.usage, systemUnitPrice: sys.unitPrice, systemAmount: Number(sys.amount.toFixed(2)), systemDiscount: sysDiscount, externalDiscount: extDiscount, finalDiscount: extDiscount, externalUsage: extUsage, externalUnitPrice: extUnit, externalAmount: Number(extAmount.toFixed(2)), finalAmount: Number(extAmount.toFixed(2)) }
  })
...
const saveSnapshotInternal = () => {
  const items = rows.value.map((r: any) => ({ productCode: r.productCode, systemAmount: r.systemAmount, externalAmount: r.externalAmount, finalAmount: r.finalAmount, finalDiscount: r.finalDiscount, reason: r.reason }))
...
const enterWriteoff = (): boolean => {
  ...
  if (!embedded.value) router.push('/budget/settlement?stage=writeoff')
...
const mockUpload = async () => {
  ...
  for (let i = 0; i < includeCount; i++) {
    ...
    csvRows.push([ form.value.month, l.productName, l.productCode, billingLabel, String(finalCount), String(discount), String(finalPaid), unit.toFixed(4), amount.toFixed(2) ].join(','))
  }
  const unknowns = [ { productName: '未知产品A', productCode: 'EXT_UNKNOWN_A', unitPrice: 0.055, paid: 1200 }, { productName: '未知产品B', productCode: 'EXT_UNKNOWN_B', unitPrice: 0.045, paid: 800 } ]
  for (const u of unknowns) {
    const amount = Number((u.unitPrice * u.paid).toFixed(2))
    csvRows.push([ form.value.month, u.productName, u.productCode, '查询', String(u.paid), '0', String(u.paid), u.unitPrice.toFixed(4), amount.toFixed(2) ].join(','))
  }
...
watch(() => props.supplierId, async (sid?: string) => { form.value.supplierId = sid || ''; if (form.value.supplierId && form.value.month) await buildRows() })
