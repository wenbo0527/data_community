    ...
    <a-drawer :visible="createVisible" :width="1000" @cancel="closeCreate" :esc-to-close="true" :mask-closable="false" title="新增数据服务">
      ...
      <div v-show="createStep === 0">
        <a-form :model="createForm" layout="vertical">
          <a-row :gutter="16">
            <a-col :span="8"><a-form-item label="服务类型" field="serviceType" required><a-select v-model="createForm.serviceType" placeholder="请选择类型"><a-option value="API">API</a-option><a-option value="文件">文件</a-option><a-option value="数据库">数据库</a-option><a-option value="平台工具">平台工具</a-option></a-select></a-form-item></a-col>
            <a-col :span="8"><a-form-item label="服务名称" field="name" required><a-input v-model="createForm.name" placeholder="请输入服务名称" /></a-form-item></a-col>
            <a-col :span="8"><a-form-item label="供应商" field="supplier"><a-input v-model="createForm.supplier" placeholder="请输入供应商" /></a-form-item></a-col>
          </a-row>
          ...
          <div style="margin-top: 16px">
            <a-space><a-button type="primary" @click="goNextFromChoose">下一步</a-button><a-button @click="closeCreate">取消</a-button></a-space>
          </div>
        </a-form>
      </div>
      <div v-show="createStep === 1">
        <a-form :model="createForm" layout="vertical">
          <a-row :gutter="16">
            <a-col :span="8"><a-form-item label="计费模式" field="billingMode"><a-select v-model="createForm.billingMode" placeholder="选择计费模式"><a-option value="per_call">按次</a-option><a-option value="monthly">按月</a-option><a-option value="tier">阶梯</a-option></a-select></a-form-item></a-col>
            <a-col :span="8"><a-form-item label="单价(元)" field="unitPrice"><a-input-number v-model="createForm.unitPrice" :min="0" :precision="2" placeholder="请输入单价" style="width: 100%" /></a-form-item></a-col>
            <a-col :span="8"><a-form-item label="状态" field="status"><a-select v-model="createForm.status" placeholder="选择状态"><a-option value="pending">待上线</a-option><a-option value="online">在线</a-option><a-option value="maintaining">维护中</a-option></a-select></a-form-item></a-col>
          </a-row>
          <a-row><a-col :span="24"><a-form-item label="描述" field="description"><a-textarea v-model="createForm.description" :rows="3" placeholder="请输入服务描述" /></a-form-item></a-col></a-row>
        </a-form>
        <a-divider>模板申请信息</a-divider>
        <div v-if="selectedTemplateKey === 'online-detail'">
          <a-form :model="flowApiForm" layout="vertical">
            <a-row :gutter="16"><a-col :span="8"><a-form-item label="身份证号" field="idNumber"><a-input v-model="flowApiForm.idNumber" /></a-form-item></a-col><a-col :span="8"><a-form-item label="手机号" field="mobile"><a-input v-model="flowApiForm.mobile" /></a-form-item></a-col><a-col :span="8"><a-form-item label="并发上限" field="qps"><a-input-number v-model="flowApiForm.qps" :min="0" style="width:100%" /></a-form-item></a-col></a-row>
          </a-form>
        </div>
        <div v-else-if="selectedTemplateKey === 'offline-task'">
          <a-form :model="flowOfflineTaskForm" layout="vertical">
            <a-row :gutter="16"><a-col :span="12"><a-form-item label="任务名称" field="taskName" required><a-input v-model="flowOfflineTaskForm.taskName" /></a-form-item></a-col><a-col :span="12"><a-form-item label="时间范围" field="dateRange"><a-input v-model="flowOfflineTaskForm.dateRange" placeholder="如: 2024-01-01 ~ 2024-01-31" /></a-form-item></a-col></a-row>
            <a-form-item label="批量文件"><a-upload :limit="1" :auto-upload="false" /></a-form-item>
          </a-form>
        </div>
        <div v-else-if="selectedTemplateKey === 'variable-backtrack'">
          <a-form :model="flowVariableBacktrackForm" layout="vertical">
            <a-row :gutter="16"><a-col :span="8"><a-form-item label="开始日期" field="startDate"><a-date-picker v-model="flowVariableBacktrackForm.startDate" style="width:100%" /></a-form-item></a-col><a-col :span="8"><a-form-item label="结束日期" field="endDate"><a-date-picker v-model="flowVariableBacktrackForm.endDate" style="width:100%" /></a-form-item></a-col><a-col :span="8"><a-form-item label="变量名" field="variables"><a-input-tag v-model="flowVariableBacktrackForm.variables" /></a-form-item></a-col></a-row>
          </a-form>
        </div>
        ...
        <div v-else-if="selectedTemplateKey === 'risk-query'">
          <a-form :model="flowRiskQueryForm" layout="vertical">
            <a-row :gutter="16"><a-col :span="8"><a-form-item label="身份证号" field="idNumber"><a-input v-model="flowRiskQueryForm.idNumber" /></a-form-item></a-col><a-col :span="8"><a-form-item label="开始日期" field="startDate"><a-date-picker v-model="flowRiskQueryForm.startDate" style="width:100%" /></a-form-item></a-col><a-col :span="8"><a-form-item label="结束日期" field="endDate"><a-date-picker v-model="flowRiskQueryForm.endDate" style="width:100%" /></a-form-item></a-col></a-row>
        ...
        <a-card :bordered="true">
          ...
          <a-table :data="pipeline" :pagination="false">
            <template #columns>
              <a-table-column title="#" :width="60"><template #cell="{ rowIndex }">{{ rowIndex + 1 }}</template></a-table-column>
              <a-table-column title="类型" :width="120"><template #cell="{ record }"><a-tag>{{ stepTypeLabel(record.type) }}</a-tag></template></a-table-column>
              <a-table-column title="名称" :width="200"><template #cell="{ record }"><a-input v-model="record.name" placeholder="环节名称" /></template></a-table-column>
              <a-table-column title="配置" :width="480"><template #cell="{ record }"><div v-if="record.type === 'datasource'"><a-row :gutter="8"><a-col :span="8"><a-input v-model="record.config.host" placeholder="主机" /></a-col><a-col :span="8"><a-input v-model="record.config.database" placeholder="数据库" /></a-col><a-col :span="8"><a-input v-model="record.config.table" placeholder="表名" /></a-col></a-row></div><div v-else-if="record.type === 'sql'"><a-input-tag v-model="record.config.tags" placeholder="标签（可选）" style="margin-bottom:8px" /><a-textarea v-model="record.config.query" :rows="3" placeholder="编写SQL语句" /></div><div v-else-if="record.type === 'python'"><a-input-tag v-model="record.config.dependencies" placeholder="依赖包（可选）" style="margin-bottom:8px" /><a-textarea v-model="record.config.script" :rows="3" placeholder="编写Python脚本" /></div><div v-else-if="record.type === 'approval'"><a-input v-model="record.config.approver" placeholder="审批人" /></div><div v-else-if="record.type === 'deploy'"><a-input v-model="record.config.env" placeholder="发布环境，如：prod" /></div><div v-else-if="record.type === 'operation'"><a-input v-model="record.config.monitor" placeholder="监控项，如：QPS/错误率" /></div></template></a-table-column>
              <a-table-column title="操作" :width="220"><template #cell="{ rowIndex }"><a-space><a-button size="mini" @click="moveUp(rowIndex)" :disabled="rowIndex===0">上移</a-button><a-button size="mini" @click="moveDown(rowIndex)" :disabled="rowIndex===pipeline.length-1">下移</a-button><a-button size="mini" status="danger" @click="removePipelineStep(rowIndex)">删除</a-button></a-space></template></a-table-column>
            </template>
          </a-table>
        </a-card>
        <div style="margin-top: 16px"><a-space><a-button @click="createStep = 0">上一步</a-button><a-button type="primary" @click="createStep = 2">下一步</a-button></a-space></div>
      </div>
      <div v-show="createStep === 2">
        <a-space style="margin-bottom: 12px"><a-checkbox v-model="needAccompany">需要陪跑计划</a-checkbox></a-space>
        <div v-if="needAccompany">
          <a-steps :current="currentStep" style="margin-bottom: 16px"><a-step v-for="s in steps" :key="s.key" :title="s.title" /></a-steps>
          <component :is="currentComponent" v-bind="currentStepProps" @next="goToNextStep" @prev="goToPreviousStep" />
        </div>
        <div style="margin-top: 16px"><a-space><a-button @click="createStep = 1">上一步</a-button><a-button type="primary" @click="submitCreate">提交</a-button></a-space></div>
      </div>
    </a-drawer>
  </div>
</template>
...
const goNextFromChoose = () => { if (!createForm.serviceType || !createForm.name || !selectedTemplate.value) { Message.warning('请选择类型并选择模板，填写服务名称'); return } createStep.value = 1 }
...
const defaultConfigFor = (t: string) => t === 'datasource' ? { host: '', database: '', table: '' }
  ...
  : {}
const addPipelineStep = () => { const type = newStepType.value || 'datasource'; pipeline.value.push({ type, name: `${stepTypeLabel(type)}_${pipeline.value.length + 1}`, config: defaultConfigFor(type) }) }
...
const accompanyForm = ref<FormData>({ workId: '', basic: { name: '', cacheTime: '30', days: 0, periods: 0, description: '', periodDays: [30] }, creditProducts: [], dataProducts: [] })
...
const currentStepProps = computed(() => {
  ...
  if (steps[currentStep.value]?.key === 'product') {
    return {
      ...base,
      modelValue: { products: accompanyForm.value.dataProducts || [], periodDays: accompanyForm.value.basic?.periodDays || [] },
      'onUpdate:modelValue': (v: any) => {
        accompanyForm.value.dataProducts = v?.products ?? []
        accompanyForm.value.basic = { ...(accompanyForm.value.basic || { name: '', cacheTime: '30', days: 0, periods: 0, description: '', periodDays: [] }), periodDays: v?.periodDays ?? [] }
      }
    }
  }
...
const submitCreate = async () => {
  ...
  const ok = editMode.value && editingId.value ? await store.updateService(String(editingId.value), payload as any) : await store.createService(payload as any)
