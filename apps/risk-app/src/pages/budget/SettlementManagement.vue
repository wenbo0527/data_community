      ...
      <a-card title="子任务进度" :bordered="true" style="margin-bottom: 12px">
        <a-table :data="subtasks" :pagination="false">
          <template #columns>
            ...
            <a-table-column title="状态" :width="120">
              <template #cell="{ record }"><a-tag :color="statusTagColor(record.status)">{{ statusLabel(record.status) }}</a-tag></template>
            </a-table-column>
...
const filters = reactive<{ suppliers: string[]; status?: TaskStatus }>({ suppliers: [] })
...
const createForm = reactive<{ supplierIds: string[]; contractIds: string[]; granularity: Granularity; timeLabel: string; tolerance: number; createdBy: string; remark?: string }>({ supplierIds: [], contractIds: [], granularity: 'month', timeLabel: `${new Date().getFullYear()}-${String(new Date().getMonth()+1).padStart(2,'0')}`, tolerance: 0, createdBy: '管理员' })
...
const statusTagColor = (s: TaskStatus) => {
  ...
  const t = window.setInterval(() => {
    ...
    subtasks.value = subtasks.value.map(st => {
      if (st.status !== 'done') {
        ...
        if (p >= 100) { status = 'done'; done += 1 } else if (p >= 66) { status = 'writeoff' } else if (p >= 33) { status = 'reconcile' }
    ...
    if (percent >= 100) { task.status = 'done'; currentStep.value = 4; window.clearInterval(t); delete timers.value[task.id]; Message.success('结算任务已完成，可生成报告') } else if (percent >= 66) { task.status = 'writeoff' } else if (percent >= 33) { task.status = 'reconcile' }
  }, 1000)
...
const applyFilter = () => {}
const resetFilter = () => { filters.suppliers = []; filters.status = undefined }
...
const exportList = () => {
  ...
  const rows = displayedTasks.value.map(t => [t.id, t.taskName || '', t.supplierIds.length, t.contractIds.length, granularityLabel(t.granularity), t.timeLabel, t.summary.budgetAmount, t.summary.actualAmount, t.summary.diffAmount, t.summary.diffRate, statusLabel(t.status), t.progress, t.createdBy, formatDate(t.createdAt)])
