<template>
  <div class="business-process-tab">
    ...
    <a-table :data="tableData" :pagination="false" :bordered="{ wrapper: true, cell: true }" :span-method="spanMethod" class="process-table">
      <template #columns>
        <a-table-column title="客群结构" data-index="customerType" :width="120">
          <template #cell="{ record, rowIndex }">
            <template v-if="rowIndex % 8 === 0">{{ record.customerType }}</template>
          </template>
        </a-table-column>
        <a-table-column title="业务流程" data-index="processName" :width="150">
          <template #cell="{ record }">{{ record.processName }}</template>
        </a-table-column>
...
const formatAmount = (n: number) => Number(n ?? 0).toLocaleString('zh-CN', { style: 'currency', currency: 'CNY' })
const formatPercent = (n: number) => `${(Number(n ?? 0) * 100).toFixed(2)}%`
const formatChangeValue = (val: number, type: 'amount' | 'number' | 'percent') => {
  if (type === 'amount') return formatAmount(val)
  if (type === 'percent') return formatPercent(val)
  return Number(val ?? 0).toLocaleString()
}
const tableData = ref([
  { customerType: '新户', processName: '授信申请量', creditApplyCount: { current: 5000, change: 500 }, rowSpan: 2 },
  { customerType: '新户', processName: '授信通过率', creditApprovalRate: { current: 0.65, change: 0.05 }, rowSpan: 0 },
  { customerType: '新户', processName: '授信平均额度', creditLimit: { current: 5000000, change: 500000 }, rowSpan: 2 },
  { customerType: '新户', processName: '支用申请量', loanApplyCount: { current: 3000, change: 300 }, rowSpan: 0 },
  { customerType: '新户', processName: '支用通过率', loanApprovalRate: { current: 0.8, change: 0.02 }, rowSpan: 2 },
  { customerType: '新户', processName: '支用金额', loanAmount: { current: 3000000, change: 300000 }, rowSpan: 0 },
  { customerType: '新户', processName: '平均期数', averageTerm: { current: 12, change: 1 }, rowSpan: 2 },
  { customerType: '新户', processName: '平均定价', averagePricing: { current: 0.15, change: 0.01 }, rowSpan: 0 },
  { customerType: '老户', processName: '授信申请量', creditApplyCount: { current: 4000, change: 400 }, rowSpan: 2 },
  { customerType: '老户', processName: '授信通过率', creditApprovalRate: { current: 0.6, change: 0.04 }, rowSpan: 0 },
  { customerType: '老户', processName: '授信平均额度', creditLimit: { current: 4000000, change: 400000 }, rowSpan: 2 },
  { customerType: '老户', processName: '支用申请量', loanApplyCount: { current: 2000, change: 200 }, rowSpan: 0 },
  { customerType: '老户', processName: '支用通过率', loanApprovalRate: { current: 0.75, change: 0.01 }, rowSpan: 2 },
  { customerType: '老户', processName: '支用金额', loanAmount: { current: 2000000, change: 200000 }, rowSpan: 0 },
  { customerType: '老户', processName: '平均期数', averageTerm: { current: 10, change: 0.5 }, rowSpan: 2 },
  { customerType: '老户', processName: '平均定价', averagePricing: { current: 0.12, change: 0.005 }, rowSpan: 0 }
])
const emitTimeRangeChange = () => { emit('time-range-change', props.timeRange) }
const getChangeClass = (value: number) => value > 0 ? 'warning-text' : 'success-text'
const spanMethod = ({ record, column, rowIndex }: { record: any; column: any; rowIndex: number }) => {
  if (column.dataIndex === 'customerType') {
    ...
    return { rowspan: isFirstRow ? sameTypeRows.length : 0, colspan: 1 }
  }
...
<style scoped>
.business-process-tab { padding: 16px; }
