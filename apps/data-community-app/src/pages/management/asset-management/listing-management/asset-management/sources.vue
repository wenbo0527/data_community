<template>
  <!-- @prd: asset-listing.data-sources -->
  <div class="asset-sources-page">
    ...
    <a-row :gutter="16" class="stats-row">
      <a-col :span="8">
        <a-card>
          <a-statistic title="资产总数" :value="stats.totalAssets" :value-style="{ color: '#165DFF' }">
    ...
    <div class="section-title">
      <h3>数据资产</h3>
      <span class="section-subtitle">点击进入查看系统下的资产列表与上下架操作</span>
    </div>
    <a-row :gutter="[16, 16]">
...
import { getSystemsByKind, mockTables, type AssetSystemId } from '@/mock-dca/data-map'
...
// 系统卡片数据 - 只展示「数据资产」分组（HIVE 数仓）
const systemCards = computed(() => {
  return getSystemsByKind('asset').map(sys => {
...
const goToSystem = (sysId: AssetSystemId) => {
  router.push(`/management/asset-management/listing-management/asset-management/system/${sysId}`)
}
</script>

<style scoped>
.asset-sources-page { padding: 16px 24px 24px; }
