  ...
  <div class="notification-detail-page">
          ...
          <a-card title="相关通知" class="info-card">
            <div class="related-list">
              <div
                ...
                :key="item.id"
                class="related-item"
                @click="$router.push(`/management/notifications/detail/${item.id}`)"
              >
    ...
    <div v-else class="error-container">
      <a-result status="404" title="通知不存在" subtitle="您访问的通知可能已被删除或不存在">
        <template #extra>
          <a-button type="primary" @click="$router.push('/management/notifications')">
            返回列表
          </a-button>
        </template>
      </a-result>
...
import { goBack } from '@/utils-dca/routerUtils'
...
import { NotificationAPI, OperationLogAPI } from '@mock-dca/api/notification'
import NotificationDetailContent from '@/components-dca/common/NotificationDetailContent.vue'
import DateUtils from '@/utils-dca/dateUtils'

const route = useRoute()
const router = useRouter()
const goBackAction = () => goBack(router, '/management/notifications')
...
const handleEdit = () => {
  router.push(`/management/notifications/edit/${route.params.id}`)
}
