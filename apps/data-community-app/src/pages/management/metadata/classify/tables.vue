<template>
  <div class="classify-tables-page">
    ...
    <PageHeader :title="`${currentSystem?.name || '数据源'} · 表列表`" :sub-title="`共 ${tables.length} 张表，${currentSystem?.description || ''}`">
      <template #extra>
        <a-space>
...
import { SENSITIVITY_COLORS } from '@/mock-shared/classify-constants'
import type { ClassifyField, ClassifyTable, SensitivityLevel } from '@/mock-shared/classify-types'
...
const goToDetail = (t: ClassifyTable) => {
  router.push(`/management/metadata/classify/table/${systemId.value}/${t.schema}/${t.table_name}`)
}
