      ...
      >
        <template #indicatorName="{ record }">
          <div class="indicator-info" @click="handleViewDetail(record)" style="cursor: pointer">
            <a-link @click.stop="goMetricDict(record)">{{ record.name }}</a-link>
...
function goMetricsMap() {
  router.push('discovery/indicator-dict')
}
function goMetricDict(record: any) {
  router.push({
    path: 'discovery/indicator-dict',
    query: { code: record.code, name: record.name }
  })
}
</script>
