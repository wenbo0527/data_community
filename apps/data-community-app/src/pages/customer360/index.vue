          ...
          <div v-if="searchResults.length">
            <div class="search-results-header">
              <span v-if="jumpCountdown > 0" class="jump-indicator">
                ...
                <a-button type="text" size="small" @click="cancelJump">取消</a-button>
              </span>
            ...
            >
              <template #columns>
                ...
                <a-table-column title="姓名" data-index="name" :width="100" fixed="left" />
                ...
                <a-table-column title="年龄/性别" :width="100">
                  <template #cell="{ record }">
                ...
                <a-table-column title="等级" :width="100">
                  <template #cell="{ record }">
                    <a-tag
                      size="small"
                      :color="record.customerLevel === 'VIP' ? 'gold' : (record.customerLevel === '白银' ? 'gray' : 'arcoblue')"
                    >
                      {{ record.customerLevel || '-' }}
                    </a-tag>
                  </template>
                </a-table-column>
                <a-table-column title="最近一笔借据" :width="280">
                  <template #cell="{ record }">
                    <div v-if="record.recentLoan" class="recent-loan">
                      <a-tag size="small" :color="record.recentLoan.status === '逾期' ? 'red' : 'green'">
                        {{ record.recentLoan.status }}
                      </a-tag>
                      <span class="mono">{{ record.recentLoan.loanNo }}</span>
                      <span class="muted">¥{{ (record.recentLoan.balance || 0).toLocaleString() }}</span>
                    </div>
                    <span v-else class="muted">无借据</span>
                  </template>
                </a-table-column>
                <a-table-column title="产品名称" data-index="productNames" :width="200">
                  <template #cell="{ record }">
                    <div class="list-tags">
...
import { IconSearch, IconInfoCircleFill } from '@arco-design/web-vue/es/icon'
