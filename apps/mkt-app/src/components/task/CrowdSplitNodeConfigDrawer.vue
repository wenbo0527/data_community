      ...
      <div class="crowd-layers">
        <div v-for="(layer, index) in formData.crowdLayers" :key="layer.id" class="crowd-layer">
          <div class="layer-header">
            ...
            <a-button v-if="formData.crowdLayers.length > 1" type="text" status="danger" size="small" class="remove-btn" @click="removeCrowdLayer(index)"><IconMinus /></a-button>
          </div>
          <div class="layer-content">
            ...
            <a-button type="text" size="small" class="search-btn"><IconSearch /></a-button>
          </div>
        </div>
      </div>
      <div class="unmatch-branch-section">
        <div class="section-title"><span>未命中分支配置</span><a-tooltip content="未命中分支用于处理不满足上述任何人群条件的用户，此分支不可删除"><IconInfoCircle class="info-icon" /></a-tooltip></div>
        <div class="crowd-layer unmatch-layer">
          ...
          <div class="layer-content">
            ...
            <a-button type="text" size="small" class="search-btn" disabled><IconSearch /></a-button>
          </div>
        </div>
      </div>
...
.info-icon { color: var(--subapp-info); cursor: help }
.unmatch-layer { background-color: #f0f8ff; border: 1px solid #b3d8ff }
.fixed-label { background-color: #e6f4ff; color: var(--subapp-info); padding: 2px 8px; border-radius: 4px; font-size: 12px }
...
.remove-btn { margin-left: auto; color: var(--subapp-danger) }
...
.search-btn:hover { background-color: var(--subapp-border); color: var(--subapp-info) }
.action-buttons { display: flex; gap: 12px }
.add-crowd-btn, .add-hit-crowd-btn { flex: 1; height: 40px; font-size: 14px; font-weight: 500; border-radius: 6px }
.add-crowd-btn { background-color: var(--subapp-info); border-color: var(--subapp-info) }
.add-crowd-btn:hover { background-color: #40a9ff; border-color: #40a9ff }
.add-hit-crowd-btn { background-color: var(--subapp-info); border-color: var(--subapp-info) }
