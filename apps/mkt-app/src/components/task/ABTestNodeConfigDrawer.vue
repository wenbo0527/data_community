        ...
        <div class="branches-container">
          <div v-for="(branch, index) in formData.branches" :key="index" class="branch-item">
            <div class="branch-header">
              ...
              <a-button v-if="formData.branches.length > 2" type="text" status="danger" size="small" @click="removeBranch(index)"><template #icon><IconDelete /></template>删除</a-button>
            </div>
          ...
          <a-button v-if="formData.branches.length < 10" type="dashed" block @click="addBranch"><template #icon><IconPlus /></template>新增分支</a-button>
        </div>
      </a-form-item>
    </template>
    <template #debug>
