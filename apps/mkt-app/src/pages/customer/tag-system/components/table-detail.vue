  ...
  <div class="table-detail">
    <a-descriptions :column="2" bordered>
      ...
      <a-descriptions-item label="ID映射状态">
        <a-space>
          <IconCheckCircle v-if="tableData.mappingStatus === 'configured'" style="color: var(--subapp-success)" />
          <IconCloseCircle v-else-if="tableData.mappingStatus === 'not_configured'" style="color: var(--subapp-danger)" />
    ...
    <div class="schema-info">
      ...
      <a-table :columns="schemaColumns" :data="mockSchemaData" size="small">
        ...
        <template #isPrimaryKey="{ record }">
          <IconCheckCircle v-if="record.isPrimaryKey" style="color: var(--subapp-success)" />
          <IconCloseCircle v-else style="color: #c9cdd4" />
        </template>
        <template #isMappingField="{ record }">
          <IconCheckCircle v-if="record.isMappingField" style="color: var(--subapp-success)" />
          <IconCloseCircle v-else style="color: #c9cdd4" />
        </template>
      </a-table>
    </div>
