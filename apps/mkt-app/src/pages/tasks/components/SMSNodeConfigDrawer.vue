...
const smsTemplateOptions = [
  { label: '营销推广模板', value: 'marketing' },
  { label: '活动通知模板', value: 'activity' },
  { label: '验证码模板', value: 'verification' },
  { label: '订单通知模板', value: 'order' },
  { label: '系统通知模板', value: 'system' }
]
...
const {
  ...
  nodeType: 'sms',
  onSubmit: (data) => {
    console.log('[SMSNodeConfigDrawer] 提交配置:', data)
    return {
  ...
  onCancel: () => {
    console.log('[SMSNodeConfigDrawer] 处理取消操作')
  }
})
...
const getSmsPreviewContent = () => {
  const templateMap = {
    marketing: '【品牌名】亲爱的用户，我们为您准备了专属优惠活动，点击链接查看详情：http://example.com 退订回T',
    activity: '【品牌名】您好，我们即将举办精彩活动，时间：{活动时间}，地点：{活动地点}，期待您的参与！退订回T',
    verification: '【品牌名】您的验证码是：{验证码}，请在5分钟内完成验证，如非本人操作请忽略。',
    order: '【品牌名】您的订单{订单号}已确认，预计{发货时间}发货，感谢您的购买！退订回T',
    system: '【品牌名】系统通知：{通知内容}，如有疑问请联系客服。退订回T'
  }
  
  return templateMap[formData.smsTemplate] || '请选择短信模板查看预览内容'
}
</script>
