...
export function createHorizontalPortConfig(outCount = 1, options = {}) {
  ...
  const groups = {
    in: {
      ...
      connectOptions: { inbound: true, outbound: false, maxConnections: 1 },
      markup: [ { tagName: 'circle', selector: 'circle' } ],
      attrs: { circle: { r: NODE_DIMENSIONS.PORT_RADIUS, magnet: true, stroke: COLORS.PORT_STROKE, strokeWidth: COLORS.PORT_STROKE_WIDTH, fill: COLORS.PORT_FILL_IN, 'data-port-type': 'in', class: 'x6-port-body' } }
    },
    out: {
      position: { name: 'absolute' },
      layout: null,
      connectOptions: { inbound: false, outbound: true, maxConnections: 1 },
      markup: [ { tagName: 'circle', selector: 'circle' } ],
      attrs: { circle: { r: NODE_DIMENSIONS.PORT_RADIUS, magnet: true, stroke: COLORS.PORT_STROKE, strokeWidth: COLORS.PORT_STROKE_WIDTH, fill: COLORS.PORT_FILL_OUT, 'data-port-type': 'out', class: 'x6-port-body' } }
    }
  }

  const items = []
  if (includeIn) {
    items.push({ id: 'in', group: 'in', args: { dy: 0 }, attrs: { circle: { 'port': 'in', 'port-group': 'in', 'data-port': 'in', 'data-port-group': 'in' } } })
  }
