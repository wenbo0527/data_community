...
const relationshipTypes = [
  { key: 'family', label: '家庭关系', color: 'var(--subapp-info)' },
  { key: 'guarantee', label: '担保关系', color: '#52c41a' },
  { key: 'co_loan', label: '共同借贷', color: '#faad14' },
  { key: 'emergency', label: '紧急联系人', color: 'var(--subapp-info)' },
  { key: 'business', label: '商业关系', color: 'var(--subapp-info)' },
...
const getNodeStyle = (node: any) => {
  ...
  let fill = 'var(--subapp-info)'
  let stroke = 'var(--subapp-info)'
  let size = isCenter ? 40 : 30
  
  // 根据风险等级设置颜色
  if (riskLevel === 'high') {
    fill = 'var(--subapp-danger)'
    stroke = 'var(--subapp-danger)'
    ...
    const node = graph.value!.addNode({
      ...
      attrs: {
        ...
        label: {
          ...
          fontSize: 12,
          fill: 'var(--subapp-text-secondary)'
        }
      },
      data: nodeData
    })
