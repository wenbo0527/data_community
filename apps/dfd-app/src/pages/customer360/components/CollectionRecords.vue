...
.empty-state {
  ...
  background: #fafbfc;
  border-radius: 8px;
  border: 2px dashed var(--subapp-border);
  margin: 20px 0;
}
...
.loading-spinner {
  width: 40px;
  height: 40px;
  border: 3px solid var(--subapp-border);
  border-top: 3px solid var(--subapp-primary);
  border-radius: 50%;
...
.skeleton-line {
  height: 16px;
  background: linear-gradient(90deg, var(--subapp-border) 25%, #e6e6e6 50%, var(--subapp-border) 75%);
...
.stat-item::before {
  ...
  right: 0;
  height: 3px;
  background: linear-gradient(90deg, var(--subapp-info), #52c41a);
  transform: scaleX(0);
...
.reset-filters-btn {
  ...
  border-color: var(--subapp-border);
  color: var(--subapp-text-secondary);
}
...
.arco-btn-primary {
  background: linear-gradient(135deg, var(--subapp-primary), #4080ff);
  border-color: var(--subapp-primary);
}
