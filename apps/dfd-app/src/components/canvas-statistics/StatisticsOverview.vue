...
const initTrendChart = () => {
  ...
  const option = {
    ...
    yAxis: {
      ...
      splitLine: { lineStyle: { color: 'var(--subapp-border)' } }
    },
    series: [{
      name: '转化率',
      type: 'line',
...
.stats-cards {
  .stat-card {
    ...
    &.primary {
      background: linear-gradient(135deg, var(--subapp-info) 0%, #69c0ff 100%);
      color: #fff;
    }
    ...
    &.info {
      background: linear-gradient(135deg, var(--subapp-info) 0%, #b37feb 100%);
      color: #fff;
    }
  }
