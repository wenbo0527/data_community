...
const renderCharts = (tasks) => {
  ...
  ChartRenderer.renderPieChart(taskTypeChart.value, taskTypeData, {
    title: '任务类型分布',
    colors: ['var(--subapp-info)', '#52c41a', '#fa8c16', 'var(--subapp-info)']
  })
  ...
  ChartRenderer.renderPieChart(taskStatusChart.value, taskStatusData, {
    title: '任务状态分布',
    colors: ['var(--subapp-info)', '#52c41a', '#fa8c16', '#f5222d']
  })
...
const renderMockCharts = (tasks) => {
  ...
  charts.forEach((chartRef, index) => {
    if (chartRef.value) {
      ...
      // 绘制占位图
      ctx.fillStyle = 'var(--subapp-border)'
...
const getProgressColor = (percentage) => {
  ...
  if (percentage >= 60) return 'var(--subapp-info)'
