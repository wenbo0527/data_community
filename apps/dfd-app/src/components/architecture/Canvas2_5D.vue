        ...
        <svg v-if="ruler.active || flags.showGuides" class="debug-layer" width="100%" height="100%" style="pointer-events: none; position: absolute; inset: 0; z-index: 25;">
          ...
          <g v-for="(pos, id) in nodesPx" :key="id">
            ...
            <text 
              ...
              fill="var(--subapp-primary)" :font-size="iconSize * 0.08" font-weight="bold" text-anchor="middle"
            >
            ...
            <!-- Edge Midpoints -->
            <circle :cx="pos.x" :cy="Number(pos.y) - iconSize/2" :r="iconSize * 0.02" fill="var(--subapp-info)" /> <!-- Top -->
            <circle :cx="pos.x" :cy="Number(pos.y) + iconSize/2" :r="iconSize * 0.02" fill="var(--subapp-info)" /> <!-- Bottom -->
            <circle :cx="Number(pos.x) - iconSize/2" :cy="pos.y" :r="iconSize * 0.02" fill="var(--subapp-info)" /> <!-- Left -->
            <circle :cx="Number(pos.x) + iconSize/2" :cy="pos.y" :r="iconSize * 0.02" fill="var(--subapp-info)" /> <!-- Right -->
            <!-- Label for ID -->
