...
// 类型映射
const typeMap = {
  numerical: { label: '数值型', color: 'var(--subapp-success)' },
  categorical: { label: '分类型', color: 'var(--subapp-info)' },
  text: { label: '文本型', color: 'var(--subapp-warning)' },
  ...
  graph.value = new Graph({
    ...
    highlighting: {
      magnetAdsorbed: {
        ...
        args: {
          attrs: {
            fill: 'var(--subapp-primary)',
            stroke: 'var(--subapp-primary)'
          }
        }
      }
    }
  ...
  try {
    ...
    filteredNodes.forEach((node, index) => {
      ...
      const nodeData = {
        ...
        attrs: {
          body: {
            fill: typeMap[node.type]?.color || 'var(--subapp-success)',
            stroke: '#000',
            strokeWidth: 1,
            rx: 8,
            ry: 8
...
const getRelationColor = (type) => {
  const colorMap = {
    dependency: 'var(--subapp-success)',
    derivation: 'var(--subapp-info)',
    reference: 'var(--subapp-warning)',
    association: '#722ed1'
  }
